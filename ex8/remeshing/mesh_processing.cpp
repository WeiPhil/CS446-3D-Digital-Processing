...
void MeshProcessing::calc_target_length(const REMESHING_TYPE &remeshing_type)
{
	...
	const float TARGET_LENGTH = 0.03;
	...
	else if (remeshing_type == CURV)
	{
		...
		float oneOverRatio_mean = 1.0f / ratio_mean;
		std::cout << "Mean ratio is " << ratio_mean << std::endl;
		std::cout << "New Target length " << TARGET_LENGTH * oneOverRatio_mean << std::endl;

		// Rescale to desired length
		for (Mesh::Vertex_iterator v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it)
		{
			target_length[*v_it] *= oneOverRatio_mean;
		}
...
void MeshProcessing::split_long_edges()
{
	...
	for (; !finished && i < MAX_IT; ++i)
	{
		...
		for (Mesh::Edge_iterator e_it = mesh_.edges_begin(); e_it != e_end; ++e_it)
		{
			...
			Scalar targetLength = (target_length[v0] + target_length[v1]) * 0.5;
			...
			if (edgeLength > ((4.f / 3.f) * targetLength))
			{
				...
				Point p = (mesh_.position(v0) + mesh_.position(v1)) * 0.5;
				Mesh::Vertex v = mesh_.add_vertex(p);

				// Interpolate normal and target length
				normals[v] = (normals[v0] + normals[v1]) * 0.5;
				target_length[v] = (target_length[v0] + target_length[v1]) * 0.5;
